...
def load_from_cfg(cfg: Config, device):
    '''
    ...
    loss_criterions = get_loss_criterion(cfg=cfg, device=device)
    trainer = get_trainer(
        cfg=cfg,
        model=model,
        optimizer=optimizer,
...
def load_model(cfg:Config, device):
    ...
    '''
    checkpoint = torch.load(cfg.checkpoint_load_path, map_location=device)
...
def resume(cfg:Config, device, model_state_dict:dict, optimizer_state_dict:dict, scheduler_state_dict:dict):
    '''
...
def get_loss_criterion(cfg:Config, device):
    criterions = []
    if 'dice' in cfg.loss:
    ...
    if 'bce' in cfg.loss:
        criterions.append(nn.BCEWithLogitsLoss(reduction=cfg.reduction, pos_weight=torch.tensor(cfg.bcd_pos_weight).to(device)))
    if cfg.loss == []:
